...
- [ ] AC1: O bloco JSX de "Modelos de planilha" (atual em `src/components/disparos/ImportListaTab.tsx` linhas ~172-194) está removido — não há mais referência aos arquivos `lista_recomendacao.csv`, `lista_pessoal.csv`, `lista_evento.csv`, `lista_network.csv` no componente.
- [ ] AC2: O bloco JSX de "Modelo de importação" / preset selector (linhas ~134-170) está removido. As dependências relacionadas também saem: `import { useImportPresets }`, `import type { ImportPreset }`, `const { data: presets = [] } = useImportPresets()`, estados `activePreset` e `leadControl`, função `applyPreset`, badge de preset ativo no estado `mapping`, e a passagem de `lead_control: leadControl ?? undefined` em `handleConfirmMapping` (passa a ser omitida).
- [ ] AC3: Imports não usados são removidos: `Layers` e `Download` (lucide-react), `Badge` (se só era usado para o preset badge), `useImportPresets`, `ImportPreset`.
- [ ] AC4: No estado `idle` o componente exibe somente o `<FileUploadZone />`. Sem nenhum bloco acima dele.
- [ ] AC5: No estado `mapping` o componente NÃO exibe mais o badge "Modelo: {nome}" — apenas as seções "Criar negócios no CRM", `<FieldMapper />`, `<ImportPreviewTable />` e os botões de ação.
- [ ] AC6: Os arquivos físicos `public/lista_recomendacao.csv`, `public/lista_pessoal.csv`, `public/lista_evento.csv`, `public/lista_network.csv` são removidos do repositório (caso existam em `/public`).
- [ ] AC7: `npm run build` e `npm run typecheck` passam sem warnings de import não usado.
- [ ] AC8: Smoke test manual: criar disparo > Audiência > tab Import > confirmar que a tela mostra APENAS o dropzone, sem dropdown de modelo nem grid de download.
- [ ] AC9: Nenhum outro componente do app referencia `useImportPresets` ou os CSVs estáticos. Confirmar com `grep -r "useImportPresets\|lista_recomendacao\|lista_pessoal\|lista_evento\|lista_network" src/ supabase/`.
