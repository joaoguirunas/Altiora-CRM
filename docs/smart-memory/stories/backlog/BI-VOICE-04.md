...
- [ ] **AC9 — Tipos e lint:** `tsc --noEmit` exit 0; `eslint src/components/bi/VoiceChatButton.tsx src/lib/voice/biTools.ts src/components/settings/BiVoiceFeatureToggle.tsx` exit 0. Sem `as any` introduzido.
- [ ] **AC10 — Story de débito documenta deferimentos:** se algum AC for tecnicamente bloqueado por outra story (ex.: AC6 backend depender de US-CFG-06 — Permissões granulares), criar nota explícita em "Notas" e não deixar AC silenciosamente unchecked.
...
| Campo      | Valor |
|---         |---|
| Agente     | dev-dev-alpha (Nova) — already in_progress per task #22 |
| Iniciado   | — |
| Concluído  | — |
| Branch     | feat/bi-voice-04-integration |
