...
- [ ] **AC1:** `CriarDisparo.tsx:97` (e `ConfiguracaoDisparoTab.tsx:45`, `CriarDisparoModal.tsx` se aplicável) só listam templates com `meta_template_name IS NOT NULL` E `meta_template_status='APPROVED'`. Templates filtrados ganham dica visual "Template ainda não publicado na Meta — configure em Configurações → Templates" + link.
- [ ] **AC2:** UI ganha campo de mapeamento `variables_map` para templates com placeholders (`{{1}}, {{2}}, ...`). `useWhatsappTemplates` retorna `template.variables.length` (de `json_data`). Wizard mostra N inputs, cada um aceita: (a) selecionar campo do CRM (`lead_field_values` por `q_field`), (b) digitar valor literal. Validação client-side bloqueia Play se algum placeholder não tem mapping nem literal.
- [ ] **AC3:** `handleAtivar`/`handleRetomar` em `Disparos.tsx:86, 105` invocam o `send-dispatch-worker` 1× para o primeiro batch (igual ao botão "Iniciar disparo" do detalhe), com tratamento de erro idêntico — `data.processed === 0` reverte para `draft` + `toast.error`. Erros do worker viram toast com mensagem clara (parsing igual ao `useSendDispatch.ts:70-78`).
- [ ] **AC4:** Smoke-test E2E:
...
- [ ] **AC5:** Variáveis de template (`variables_map`) são gravadas em `sends_contacts.variables_map` no insert do contato (`useSendMutations.ts:49-111`). Não quebra contratos existentes do worker (worker já lê o campo se presente).
- [ ] **AC6:** Type-check passa, lint passa, smoke E2E em ambiente de desenvolvimento documentado em screenshots/gif na PR.
