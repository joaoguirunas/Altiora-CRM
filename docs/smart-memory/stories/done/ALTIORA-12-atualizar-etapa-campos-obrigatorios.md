...
- `src/hooks/useAltioraStageTransition.ts` — ALTIORA_STAGE UUIDs, ALTIORA_STAGE_ORDER, STAGE_REQUIREMENTS (async DB checks per stage), isSkippingStages, checkStageRequirements, useConfirmarTransicao (mutation: update lead + insert lead_stage_history + insert altiora_lead_interactions)
- `src/components/negocios/AltioraTransicaoModal.tsx` — Modal com checklist de pré-requisitos, aviso de salto de etapas, confirmação explícita, retorno de etapa
- `src/pages/NegocioSingle.tsx` — handleStageClick intercepta pipeline Altiora, abre transicaoModal; AltioraTransicaoModal renderizado com onSuccess callback
- `supabase/migrations/20260725210000_altiora_stage_history.sql` — Tabela lead_stage_history com RLS

AC implementados:
- [x] AC1: Modal exibe campos faltantes antes de confirmar
- [x] AC2: Mapa de requisitos por etapa (reuniões, resultado, data_emissao)
- [x] AC3: Salto de etapas detectado + aviso + checkbox de confirmação explícita
- [x] AC4: Migration lead_stage_history — useConfirmarTransicao insere registro
- [x] AC5: Retorno a etapa anterior permitido com aviso de preservação de dados
