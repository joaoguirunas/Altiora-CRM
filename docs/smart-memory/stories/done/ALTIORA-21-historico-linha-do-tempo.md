...
## Acceptance Criteria
- [x] AC1: Aba "Histórico" na ficha do referral exibe eventos em ordem cronológica reversa (mais recente no topo) com: tipo de evento (ícone + label), descrição, ator (nome do usuário), data/hora relativa ("há 2h", "ontem").
- [x] AC2: Tipos de evento exibidos: mudança de etapa (de → para), primeiro contato registrado, reunião agendada/realizada/no-show, atribuição/reatribuição de Closer, encerramento como Perdido, Ganho, Reabertura, campos críticos alterados (valor do prêmio, produto).
- [x] AC3: Linha do tempo é carregada de `altiora_lead_interactions` — sem dados mockados. (lead_stage_history eventos também chegam via type='stage_changed' nesta tabela)
- [x] AC4: Histórico é imutável — nenhum evento pode ser deletado pela UI (apenas Admin via acesso direto ao banco em casos excepcionais).
- [x] AC5: Linha do tempo carrega os últimos 50 eventos por padrão com botão "Ver mais" para paginação (sem scroll infinito no V1).
...
## File List
- `src/hooks/useAltioraTimeline.ts` — criado: hook paginado para altiora_lead_interactions
- `src/components/negocios/AltioraTimeline.tsx` — criado: componente de linha do tempo com ícones e "Ver mais"
- `src/components/negocios/NegocioSidebar.tsx` — modificado: aba Histórico adicionada ao pipeline Altiora
