...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Default WhatsApp channel (once per invocation) for the persisted message row.
    const { data: waCh } = await supabase
      .from('settings_whatsapp_channels')
      .select('phone_number_id')
      .eq('is_default', true).eq('active', true).maybeSingle();
    const defaultPhoneId = (waCh as { phone_number_id: string } | null)?.phone_number_id ?? null;
    ...
    for (const job of jobs as JobRow[]) {
      ...
      // Template category → opt-in gate
      const { data: tpl } = await supabase
        .from('whatsapp_templates')
        .select('json_data')
        .eq('name', job.template_id).maybeSingle();
      ...
      const { data: msg, error: msgErr } = await supabase
        ...
        .insert({
          ...
          status: 'pending',
          content: contentText,
          wa_phone_number_id: defaultPhoneId ?? undefined,
          metadata: {
      ...
      try {
        const res = await fetch(`${supabaseUrl}/functions/v1/whatsapp-outbound`, {
          ...
          body: JSON.stringify({
            to: toNumber,
            ...(defaultPhoneId ? { phone_number_id: defaultPhoneId } : {}),
            people_id: job.people_id,
