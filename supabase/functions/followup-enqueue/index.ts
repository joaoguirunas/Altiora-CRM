...
serve(async (req) => {
  ...
  try {
    ...
    if (source_type === 'stage') {
      ...
      // Buscar score do lead para filtro por score_matrix_id
      let leadScoreMatrixId: string | null = null;
      if (lead.people_id) {
        const { data: scoreData } = await supabase
          .from('lead_field_values')
          .select('value')
          .eq('entity_id', lead.people_id)
          .eq('field_key', 'score_matrix_id')
          .maybeSingle();
        leadScoreMatrixId = scoreData?.value ?? null;
      }
