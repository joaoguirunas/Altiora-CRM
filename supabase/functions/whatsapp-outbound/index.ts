...
function normalise(item: string | OutboundMessageObj): NormMsg {
  if (typeof item === 'string') {
    return { type: 'text', text: item, media_url: null, filename: null, mime_type: null };
  }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (channel_id) {
      ...
      if (channel) {
        const chToken = (channel as { phone_number_id: string; access_token: string | null }).access_token;
        accessToken = chToken || envAccessToken;
        phoneNumberId = (channel as { phone_number_id: string; access_token: string }).phone_number_id;
        if (!chToken) console.warn(`whatsapp-outbound: channel_id ${channel_id} has no access_token, using env fallback`);
      } else {
    ...
    if (!phoneNumberId || !accessToken) {
      ...
      if (defaultChannel) {
        const defToken = (defaultChannel as { phone_number_id: string; access_token: string | null }).access_token;
        accessToken = defToken || envAccessToken;
        phoneNumberId = (defaultChannel as { phone_number_id: string; access_token: string }).phone_number_id;
        if (!defToken) console.warn('whatsapp-outbound: default channel has no access_token, using env fallback');
        console.log(`whatsapp-outbound: using default channel phone_number_id=${phoneNumberId}`);
      }
    }

    // 2. Channel used in person's last inbound message (wa_phone_number_id column)
    if ((!phoneNumberId || !accessToken) && people_id) {
      ...
      if (lastPhoneId) {
        const { data: channel } = await supabase
          .from('settings_whatsapp_channels')
          .select('phone_number_id, access_token')
          .eq('phone_number_id', lastPhoneId)
          .eq('active', true)
          .maybeSingle();

        if (channel) {
          const inbToken = (channel as { phone_number_id: string; access_token: string | null }).access_token;
          accessToken = inbToken || envAccessToken;
          phoneNumberId = (channel as { phone_number_id: string; access_token: string }).phone_number_id;
          if (!inbToken) console.warn(`whatsapp-outbound: inbound channel ${lastPhoneId} has no access_token, using env fallback`);
          console.log(`whatsapp-outbound: resolved channel from last inbound msg phone_number_id=${phoneNumberId}`);
        }
      }
    }
    ...
    if (!phoneNumberId || !accessToken) {
      ...
      if (anyChannel) {
        const anyToken = (anyChannel as { phone_number_id: string; access_token: string | null }).access_token;
        accessToken = anyToken || envAccessToken;
        phoneNumberId = (anyChannel as { phone_number_id: string; access_token: string }).phone_number_id;
        if (!anyToken) console.warn('whatsapp-outbound: fallback channel has no access_token, using env fallback');
        console.log(`whatsapp-outbound: using fallback channel phone_number_id=${phoneNumberId}`);
      }
    }
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      if (typeof rawItem === 'object' && (rawItem as any).type === 'interactive') {
        ...
        const iResult = await sendInteractiveToMeta(accessToken, phoneNumberId, to, item.body, item.buttons ?? []);
