    ...
    switch (name) {
      ...
      case 'iniciar_conversa_whatsapp': {
        ...
        const { data: waChannel } = await supabase
          ...
          .eq('active', true)
          .maybeSingle();
        if (!waChannel?.phone_number_id) return 'Error: no active default WhatsApp channel configured';
      ...
      case 'enviar_opcoes_horario': {
        ...
        const phoneNumberId = ctx.wa_phone_number_id ?? '';
        const toNumber = ctx.whatsapp ?? '';
        if (!phoneNumberId || !toNumber) return 'Error: missing WhatsApp phone context (wa_phone_number_id or whatsapp)';
