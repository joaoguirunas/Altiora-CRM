  ...
  try {
    ...
    if (validate_only) {
      if (isWhatsApp) {
        ...
        const isValid = !!(waChannelObj?.phone_number_id && waChannelObj?.access_token);
        return new Response(
          JSON.stringify({
            success: isValid,
    ...
    for (const contact of contacts) {
      ...
      try {
        if (isWhatsApp) {
          ...
          // Resolve Meta template name — must be the name registered on Meta, never a UUID or empty
