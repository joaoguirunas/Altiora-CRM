...
async function handleInstagramDMs(
  ...
  for (const entry of entries) {
    ...
    for (const event of events) {
      ...
      if (existingPerson) {
        personId = existingPerson.id;
        log.info('ig_person_found', { person_id: personId, instagram_id: senderId });

        // Always enrich to keep profile_pic, followers, verified up to date
        await enrichInstagramProfile(senderId, personId, supabase, log, recipientId);

        // Update which IGBID received this person's messages (for multi-account routing)
        if (recipientId) {
          await supabase
            .from('clients_people')
            .update({ instagram_business_id: recipientId })
            .eq('id', personId);
        }
      } else {
        const { data: newPerson, error: insertErr } = await supabase
          ...
          .insert({
            ...
            instagram_id: senderId,
            instagram_business_id: recipientId || null,
            status: 'active',
            service_status: 'open',
          })
          .select('id')
        ...
        if (insertErr || !newPerson) {
          log.error('ig_person_insert_failed', { error: insertErr?.message, sender_id: senderId });
          continue;
        }

        personId = newPerson.id;
        log.info('ig_person_created', { person_id: personId, instagram_id: senderId });
...
Deno.serve(async (req: Request) => {
  ...
  // Echo messages (is_echo: true) are sent-message confirmations from Meta.
  // Meta may sign them with a different app secret; they carry no actionable data.
  if (rawBody.includes('"is_echo"')) {
    return new Response(JSON.stringify({ ok: true, skipped: 'echo' }), {
      status: 200,
