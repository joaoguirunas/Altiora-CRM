...
# PROJECT_ID must be set via env var — never hardcoded in the template base.
# Set it before running: export SUPABASE_PROJECT_ID=your-project-ref
PROJECT_ID="${SUPABASE_PROJECT_ID:-}"
LOG_FILE="$(dirname "$0")/.migration-watcher.log"
PID_FILE="$(dirname "$0")/.migration-watcher.pid"

if [[ -z "$PROJECT_ID" ]]; then
  echo "❌  SUPABASE_PROJECT_ID is not set. Export it before running this script:"
