...
import { Plus, Edit, Shield, ShieldCheck, Trash2, UserCircle, RefreshCw, Calendar, Video, ShieldOff, Ban, RotateCcw } from 'lucide-react';
...
export const UsuariosConfig = () => {
  ...
  /**
   * ALTIORA-23 AC4/AC5: Controle de bloqueio de conta.
   * Bloquear: active=false via useUpdateUser.
   * TODO ALTIORA-23 AC5: invalidar sessão via Edge Function admin-revoke-session
   * quando a função for criada no control plane.
   */
  const [blockTarget, setBlockTarget] = useState<{ id: string; name: string; currentActive: boolean } | null>(null);
  ...
  /** ALTIORA-23 AC4/AC5: toggle active status */
  const handleToggleBlock = async () => {
    if (!blockTarget) return;
    setIsTogglingBlock(true);
    try {
      await updateUser.mutateAsync({ id: blockTarget.id, active: !blockTarget.currentActive });
      // TODO ALTIORA-23 AC5: invocar Edge Function admin-revoke-session para
      // invalidar sessão ativa quando active=false é definido.
      // await supabase.functions.invoke('admin-revoke-session', { body: { userId: blockTarget.id } });
      toast.success(
        blockTarget.currentActive
          ? `${blockTarget.name} bloqueado com sucesso.`
          : `${blockTarget.name} reativado com sucesso.`
      );
    } catch (err: unknown) {
  ...
  return (
    <div className="space-y-5">
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-1 flex-shrink-0 ml-3">
                  ...
                  {/* ALTIORA-23 AC4/AC5: Bloquear / Reativar conta */}
                  {canEditUser && (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          variant="ghost"
                          size="sm"
                          aria-label={isAtivo ? `Bloquear ${nome}` : `Reativar ${nome}`}
                          onClick={() => setBlockTarget({ id: usuario.id, name: nome, currentActive: !!isAtivo })}
                          className={cn(
                            "h-[30px] w-[30px] p-0",
                            isAtivo
                      ...
                      <TooltipContent className="text-xs">
                        {isAtivo ? 'Bloquear conta' : 'Reativar conta'}
                      </TooltipContent>
                    </Tooltip>
                  )}
      ...
      />
      {/* ALTIORA-23 AC4/AC5: Confirmação de bloqueio/reativação */}
      <AlertDialog open={!!blockTarget} onOpenChange={() => setBlockTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="text-[15px]">
              {blockTarget?.currentActive
                ? `Bloquear ${blockTarget?.name}?`
                : `Reativar ${blockTarget?.name}?`}
            </AlertDialogTitle>
            <AlertDialogDescription className="text-[13px] leading-relaxed">
              {blockTarget?.currentActive ? (
                <>
                  O usuário <span className="font-semibold text-foreground">{blockTarget?.name}</span> será
                  bloqueado e não poderá mais acessar o sistema. A sessão atual será encerrada.
                </>
              ) : (
                <>
                  O usuário <span className="font-semibold text-foreground">{blockTarget?.name}</span> será
                  reativado e poderá acessar o sistema novamente.
                </>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setBlockTarget(null)}>Cancelar</AlertDialogCancel>
            <AlertDialogAction
              onClick={handleToggleBlock}
              disabled={isTogglingBlock}
              className={blockTarget?.currentActive
                ? "bg-destructive hover:bg-destructive/90 text-white"
                : "bg-emerald-600 hover:bg-emerald-700 text-white"}
            >
              {isTogglingBlock
                ? 'Aguarde...'
                : (blockTarget?.currentActive ? 'Bloquear' : 'Reativar')}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
