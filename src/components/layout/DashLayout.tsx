...
const DashLayout = () => {
  ...
  return (
      ...
      {/* Sidebar */}
      <aside className={cn(
        "h-screen sticky top-0 bg-sidebar border-r border-sidebar-border flex flex-col transition-[width] duration-300 ease-in-out",
      ...
      <div className="flex-1 flex flex-col min-h-0 min-w-0">
        ...
        <header className="h-[72px] bg-card border-b border-border flex items-center justify-between px-6">
          ...
          <div className="flex items-center gap-4">
            {/* Notifications */}
            <Button variant="ghost" size="icon" className="relative h-[30px] w-[30px] rounded-[4px] transition-all duration-300">
              <Bell className="w-4 h-4" />
              <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-[#00D26A] rounded-full"></span>
            </Button>
