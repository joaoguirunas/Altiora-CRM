...
export const MobileConversaItem = ({ conversa }: ConversaItemProps) => {
  ...
  const hasUnread = conversa.total_mensagens_nao_lidas > 0;
  
  // Verifica se a última mensagem é do cliente (não respondida)
  const isUnanswered = conversa.ultima_mensagem?.from_message === 'cliente';
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2 mb-1">
          <div className="flex items-center gap-2 min-w-0 flex-1">
            {/* Indicador de não respondida */}
            {isUnanswered && (
              <span className="w-2 h-2 bg-destructive rounded-full flex-shrink-0 animate-pulse" />
            )}
            <h3 className={cn(
              "text-base truncate",
        ...
        <div className="flex items-center gap-2">
          ...
          {hasUnread && (
            <Badge className="bg-primary text-primary-foreground text-micro px-2 py-0 h-5">
              {conversa.total_mensagens_nao_lidas}
            </Badge>
          )}
        </div>
      </div>
