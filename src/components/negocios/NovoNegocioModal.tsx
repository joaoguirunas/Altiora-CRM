...
interface NovoNegocioModalProps {
  ...
  stageId?: string | null;
  /** Label da entidade do pipeline (ex: "Negócio" ou "Referral"). */
  entityLabel?: string;
}
...
const NovoNegocioModal = ({ open, onOpenChange, stageId, entityLabel = "Negócio" }: NovoNegocioModalProps) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (activeTab === "criar-cliente") {
        ...
        // Verificando se selecionou uma pessoa
        if (!personId) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto rounded-[2px]">
        <DialogHeader>
          <DialogTitle className="text-lg font-semibold text-foreground">Novo {entityLabel}</DialogTitle>
          <DialogDescription className="text-sm text-muted-foreground">
            Escolha um cliente existente ou crie um novo cliente para o {entityLabel.toLowerCase()}.
          </DialogDescription>
        </DialogHeader>
