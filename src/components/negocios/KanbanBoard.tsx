...
interface KanbanBoardProps {
  ...
  dataFim?: string;
  pipelineId?: string;
  /** Nome do pipeline selecionado — usado para derivar isAltiora (ALTIORA-02). */
  pipelineName?: string;
...
const KanbanBoard = ({
  ...
  // Determina se é pipeline Altiora para renderização condicional (ALTIORA-02/03)
  const isAltiora = isAltioraPipeline(pipelineName);
  ...
  return (
    <DragDropContext onDragEnd={handleDragEnd} key={displayStages.map(s => s.id).join('-')}>
      {/* AC5 (ALTIORA-02): overflow-x-auto garante scroll horizontal em mobile para 13+ colunas */}
      <div className="flex-1 min-h-0 bg-background overflow-hidden relative" role="region" aria-label="Pipeline Kanban">
        <div className="h-full overflow-x-auto px-4 py-3" style={{ WebkitOverflowScrolling: 'touch' }}>
