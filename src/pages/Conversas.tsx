...
const Conversas = () => {
  ...
  }), [selectedTenantId, searchTerm, filtroStatusAtendimento, filtroAtendimentoIA, dateRange, filtroPipeline, filtroEtapa, filtroResponsavel, filtroTimeVendas, isManager, currentUserId]);
  const {
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {pessoaSelecionada && pessoaAtual ? <>
            ...
            <div className="flex-1 overflow-y-auto min-h-0 bg-background">
              ...
              return (
                <div key={conversa.id} className={`flex ${isFromClient ? 'justify-start' : 'justify-end'}`}>
...
const PessoaListItem = ({
  ...
  // Verifica se a última mensagem é do cliente (não respondida)
  const isUnanswered = pessoa.ultima_mensagem?.from_message === 'cliente';
  const initial = pessoa.nome?.charAt(0)?.toUpperCase() || '?';
  const timeLabel = pessoa.ultima_mensagem
    ? formatTimeAgo(pessoa.ultima_mensagem.created_at, t)
    ...
    >
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex-1 min-w-0 pt-0.5">
          {/* Linha 1: nome + badges + tempo + dot não respondida */}
          <div className="flex items-center justify-between gap-1.5">
            <div className="flex items-center gap-1.5 min-w-0">
              <span className="text-[13px] font-medium leading-tight truncate">{pessoa.nome}</span>
              <div className="shrink-0">{getChannelBadge()}</div>
              {pessoa.ai_enabled && (
                <Bot className="w-3 h-3 text-violet-500 shrink-0" title="IA ativa" />
              )}
            </div>
