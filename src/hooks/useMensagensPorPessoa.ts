...
export const useMensagensPorPessoa = (pessoaId?: string, tenantId?: string) => {
  ...
  const queryResult = useQuery({
    ...
    queryFn: async (): Promise<MensagemDetalhada[]> => {
      ...
      // Resolve merged_into chain: se a pessoa selecionada foi merged em outra,
      // o histórico de mensagens pode estar atrelado tanto ao id selecionado quanto
      // ao canonical. Buscamos no canonical para garantir histórico completo.
      // Cadeia recursiva (A→B→C) com safety limit pra prevenir loop.
      let effectivePessoaId = pessoaId;
      const visited = new Set<string>([pessoaId]);
      const MAX_MERGE_CHAIN = 5;
      for (let depth = 0; depth < MAX_MERGE_CHAIN; depth++) {
        const { data: personStatus, error: lookupErr } = await supabase
          .from('clients_people')
          .select('id, status, merged_into_id')
          .eq('id', effectivePessoaId)
          .maybeSingle();

        if (lookupErr) {
          // Lookup falhou — degrade gracioso para o pessoaId original e segue.
          // Não bloqueia o usuário; pior caso vê histórico incompleto se houver merge.
          console.error('❌ MENSAGENS: lookup merge chain falhou', lookupErr);
          break;
        }

        if (!personStatus || personStatus.status !== 'merged' || !personStatus.merged_into_id) {
          break;
        }

        const next = personStatus.merged_into_id;
        if (visited.has(next)) {
          console.error('❌ MENSAGENS: ciclo detectado na cadeia de merge', { pessoaId, next });
          break;
        }
        visited.add(next);
        effectivePessoaId = next;
        console.warn(
          `↪️ MENSAGENS: pessoa ${pessoaId} merged → ${effectivePessoaId} (depth=${depth + 1})`
        );
      }
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      const idsToWatch = new Set<string>([pessoaId]);

      // Resolve cadeia recursiva mesma lógica da query (max 5 levels, ciclo guard).
      let currentId = pessoaId;
      const visited = new Set<string>([pessoaId]);
      for (let depth = 0; depth < 5; depth++) {
        if (cancelled) return;
        const { data: personStatus, error } = await supabase
          .from('clients_people')
          .select('status, merged_into_id')
          .eq('id', currentId)
          .maybeSingle();

        if (error || !personStatus || personStatus.status !== 'merged' || !personStatus.merged_into_id) break;
        const next = personStatus.merged_into_id;
